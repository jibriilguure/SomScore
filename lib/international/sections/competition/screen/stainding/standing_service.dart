...
class StandingService {
  ...
  Future<List<LeagueStanding>> fetchLeagueStandings(
    ...
    var leagueBox =
        await Hive.openBox<LeagueStanding>('leagueStandings_$leagueId');
    var metadataBox =
        await Hive.openBox('metadata'); // Store metadata like fetch time

    // Check if data is cached and the cache is valid
    if (leagueBox.isNotEmpty && _isCacheValid(metadataBox)) {
      // Return cached data
    ...
    if (response.statusCode == 200) {
      ...
      final List<LeagueStanding> standings =
          data.map((json) => LeagueStanding.fromJson(json)).toList();
  ...
  Future<List<CupStanding>> fetchCupStandings(
      int leagueId, String season) async {
    var cupBox = await Hive.openBox<CupStanding>('cupStandings_$leagueId');
    var metadataBox =
        await Hive.openBox('metadata'); // Store metadata like fetch time

    // Check if data is cached and the cache is valid
    if (cupBox.isNotEmpty && _isCacheValid(metadataBox)) {
      // Return cached data
